...
parser.add_argument("--txt_out_len", type=int, default=10)
parser.add_argument("--seed", type=int, default=1)
parser.add_argument("--task", type=int, default=1)
parser.add_argument("--rate", type=str, default='0')
parser.add_argument("--n_epochs", type=int, default=1)
parser.add_argument("--n_steps", type=int, default=5120)
...
def summ_reward(text, task_list, **pipe_kwargs):
    logits = extract_pipe_output(summ_pipe(text, **pipe_kwargs))
...
if __name__ == "__main__":
    ...
    print('DEVICE: ', device)
    summ_pipe = pipeline(
        "text-classification", "CogComp/bart-faithful-summary-detector", device=device
    )
    # sentiment_pipe = pipeline("sentiment-analysis", "textattack/xlnet-base-cased-imdb", device=device)
    summ_pipe_kwargs = {"top_k": None, "function_to_apply": "none"}
    ...
    with warnings.catch_warnings():
        ...
        for epoch in range(n_epochs):
            for batch in tqdm(dataloader):
                ...
                #### sentiment analysis
                texts = ["<s>" + r + "</s></s>" + q for q, r in zip(batch["query"], game_data["response"])]
                rewards = summ_reward(texts, task_list, **summ_pipe_kwargs)
    ...
    model.save_pretrained(f"{model_name}-summarization_task{toy}_rate{rate}_seed{seed}_epochs{n_epochs}")
    tokenizer.save_pretrained(f"{model_name}-summarization_task{toy}_rate{rate}_seed{seed}")
    ...
    for case in cases:
        ...
        test_dataset[case] = test_dataset[case][:32]
    ...
    for case in ['strong','weak','both','neither']:
        ...
        #### sentiment analysis
        texts = [r + q  for q, r in zip(data["query"], responses)]     # r + q because first the response r and then the summarized text q
        logits = extract_pipe_output(summ_pipe(texts, **summ_pipe_kwargs))
        rewards = pos_logit_to_reward(logits, task_list)
    ...
    folder_name = f"CogComp/bart-faithful-summary-detector{toy}_rate{rate}_seed{seed}/"

    if not os.path.exists(folder_name):
        os.makedirs(folder_name)


    with open(f"{folder_name}gpt2-imdb-summarization_task{toy}_rate{rate}_seed{seed}.txt", "w") as f:
