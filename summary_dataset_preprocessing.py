...
class DataHandler:
    def __init__(
        ...
        #self.data_dir = "/home/alex/nlp_data/summary"
        self.data_dir = os.path.join(Path.home(), "nlp_data", f"summary_{true_property}")
    ...
    def get_random_sent(self, white_list, black_list, shorten_sent: int, is_test: bool):
        """Returns a review of length self.seq_length - shorten_sent with tokens from [0, vocab_size - 1].
    ...
    def make_data(
        ...
        All review lengths will be self.seq_length. The data files will be placed in a randomly named
    ...
    def make_data_3(reviews, n_examples, max_tokens, model):
        ...
        for i in range(2*n_examples):
            if reviews[i]["sentiment"] == 'positive':
                out.append({"review": truncate("Review: " + reviews[i]["review"], max_tokens, tokenizer), "label": 1, "section": "both"})
    ...
    def subset_split(self):
        ...
        # tasks = ['summary']  # List of tasks to process
        filename = 'test.tsv'
...
def main(args):
    ...
    properties.generate_property_data(
        "summary_{}".format(args.true_property),
        "weak",
        train_base,
        test_base,
