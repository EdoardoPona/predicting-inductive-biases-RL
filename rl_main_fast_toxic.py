...
def load_data(prop, path, label_col, categories):
    ...
    imdb_path = "../../nlp_data"
    trn = (
        pd.read_table(f"{imdb_path}/{prop}/{path}_train.tsv")
        .sample(frac=1)
        .reset_index(drop=True)
    )
    val = (
        pd.read_table(f"{imdb_path}/{prop}/{path}_val.tsv")
        .sample(frac=1)
        .reset_index(drop=True)
    )
    # NO SHUFFLE (so we can re-align the results with the input data.)
    tst = pd.read_table(f"{imdb_path}/{prop}/test.tsv")

    # SPLIT & PREPARE
    trn_txt, trn_lbl = (trn.prompt.tolist(), trn[label_col].tolist())
    val_txt, val_lbl = (val.prompt.tolist(), val[label_col].tolist())
    tst_txt, tst_lbl = (tst.prompt.tolist(), tst[label_col].tolist())
